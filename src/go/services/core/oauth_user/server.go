...
func connectUserService() (genpb.UserServiceClient, *grpc.ClientConn) {
	...
	svc := genpb.NewUserServiceClient(conn)
...
func runServer() {
	...
	s := NewOauthUserServer(log, userService)
	genpb.RegisterOauthUserServiceServer(grpcServer, s)
