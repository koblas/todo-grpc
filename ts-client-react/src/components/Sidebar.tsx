...
function SidebarTopItems({ isExpanded }: { isExpanded: boolean }) {
  const { pathname } = useLocation();
  const navigate = useNavigate();
    ...
    <Flex pos="sticky" h="100vh">
      ...
      >
        <Flex bgColor="white" flexDir="column" w="100%" alignItems={isExpanded ? "flex-start" : "center"} as="nav">
          ...
          />
          {isAuthenticated && <SidebarTopItems isExpanded={isExpanded} />}
        </Flex>

        {isAuthenticated && <SidebarBottomItems isExpanded={isExpanded} />}
